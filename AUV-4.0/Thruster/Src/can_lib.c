...
uint32_t CAN_RecvMsg(uint32_t RxFifo, uint8_t recvMsg[], CAN_RxHeaderTypeDef* RxHeader)
{
...
void CAN_InitFilter(void)
{
   ...
   FilterConfig.FilterIdHigh = 1; // eg: 16 << 5;	//16, First 11 MSB bits of the 16 bit register
   FilterConfig.FilterIdLow = 0; // eg: 17 << 5;	//17
   FilterConfig.FilterMaskIdHigh = 0xFFFF;  // eg: 0x7FF << 5; All bits must match
   FilterConfig.FilterMaskIdLow = 0;  //eg: 0x7FF << 5;	All bits must match
   FilterConfig.FilterFIFOAssignment = CAN_RX_FIFO0;
   FilterConfig.FilterBank = 0;
   FilterConfig.FilterMode = CAN_FILTERMODE_IDMASK; //ID mode
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	uint32_t fill_level;
	fill_level = CAN_RecvMsg(CAN_RX_FIFO0, recv_databuf, &RxHeader);
...
void HAL_CAN_RxFifo1MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	// recv_databuf is library-wide data buffer
	uint32_t fill_level;
	fill_level = CAN_RecvMsg(CAN_RX_FIFO1, recv_databuf, &RxHeader);

...
void loop(){
  ...
  if(millis() - th_timeout > 15000){
    internalStats[TH_TEMP1] = 255;
    internalStats[TH_TEMP2] = 255;
    th_timeout = millis();
  }

  if(millis() - bp_timeout > 15000){
...
//		write_SD();
      ...
      for (int i = 1; i < 8; i++){
        if((millis() - heartbeat_timeout[i]) > 2000){
	...
	// 		if(u == 'S'){
	// 		  screen.writeReg(0x8E, 0x80 | 0x00);
	// 		}else if(u == 'L'){
	// 		  char string2[20] = "LEFT";
	// 		  screen.textSetCursor(300, 200);
	// 		  screen.textTransparent(RA8875_YELLOW);
	// 		  screen.textEnlarge(4);
	// 		  screen.textWrite(string2);
	// 		  //Serial.println("LEFT");
	// 		}else if(u == 'F'){
...
void screen_init(){
	...
	screen.textWrite("Hello, how are you?");
}

...
void setup () {
	...
	currentTime = heartbeat_loop = millis();
...
START_INIT:
    ...
    CAN.init_Filt(0, 0, CAN_manipulator);
}
	...
	for (val=1; val<16; val<<=1) {
		switch (enable & val) {
			case 1: 
				//acoustic
				for (i=1;i<5;i<<=1) {
					switch (mani_state & i) {	// enabled acoustics and checking for rotary and linear
						case 1:
							// m1 is on
						case 2:
							// m2 is on

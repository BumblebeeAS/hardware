...
TempAD7414::TempAD7414(int fig, int i2c_address)
{
   configRegister = fig;
   address = i2c_address;
}
...
{
  Wire.begin();
  Wire.beginTransmission(address);         // device address AD7414-0 AS pin connects to GND (0b 1001001)\
...
{
    Wire.requestFrom(address,2,1);                // Requesting two bytes
